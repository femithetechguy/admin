...
if (
  ...
  window.firebaseAuth.Auth.Persistence
) {
  // Set persistence to local (default, but explicit)
  window.firebaseAuth
    .setPersistence(window.firebaseAuth.Auth.Persistence.LOCAL)
    .catch(() => {});
}
...
fetch("json/app.json")
  ...
  .then((appData) => {
    ...
    const dashboardPage = (appData.pages || []).find(
      (p) => p.name === "Dashboard"
    );
...
function renderAppUI() {
  ...
  if (!config || !config.appName) {
    throw new Error(
      "config must be initialized with appName before calling renderAppUI()"
    );
  }
  appRoot.innerHTML = `
...
function renderTabContent(tab, idx) {
  ...
  if (tab.name === "Applied" && window.renderAppliedTab)
    return window.renderAppliedTab(tab);
  if (
    (tab.name === "Masterclass" || tab.name === "Videos") &&
  ...
  if (tab.name === "Python" && window.renderPythonTab)
    return window.renderPythonTab(tab);
  ...
  if (tab.name === "DataWarehouse" && window.renderDataWarehouseTab)
    return window.renderDataWarehouseTab(tab);
  if (tab.name === "School" && window.renderSchoolTab)
    return window.renderSchoolTab(tab);
  if (tab.name === "Extras" && window.renderExtrasTab)
    return window.renderExtrasTab(tab);
    ...
    tabsMenu.querySelectorAll(".tab-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        if (!isNaN(idx) && tabs[idx]) {
          const tabName =
            typeof tabs[idx].name === "string"
              ? tabs[idx].name.trim().toLowerCase()
              : "";
...
function handleAuthAndTabsReady() {
  console.log("[DEBUG] handleAuthAndTabsReady", {
    authUser,
    tabs,
    selectedTabIdx,
  });
  if (authUser) {
    ...
    console.log("[DEBUG] Rendering dashboard for user", {
      selectedTabIdx,
      tabs,
    });
    renderAppUI();
