...
window.renderSQLTab = function() {
  setTimeout(() => {
    fetch('json/videos/sql.json')
      .then(r => r.json())
      .then(data => renderVideos(data.videos))
      .catch(() => {
        const el = document.getElementById('videos-content');
        if (el) {
          el.innerHTML = '<div class="text-red-500">Failed to load SQL videos.</div>';
        }
      });
  }, 0);
  return '<div class="w-full flex justify-center py-4"><span class="text-xl font-bold">SQL & Database Tutorials</span></div>' +
          ...
          ${videos.map((v, i) => {
            ...
            return `
              <tr class="border-b transition-transform duration-300 ease-out opacity-0 translate-y-4 hover:scale-[1.015] hover:bg-blue-50 video-fadein" style="animation-delay:${i*60}ms">
                ...
                <td class="px-4 py-2 text-sm">${v.Channel}</td>
